...
10. **参考材料**（解析结果的 doc_type=reference，如合同/报价单/验收单，fields.note 为提取的全文）：
    这是证据类文件而非发票——从文本理解业务要素（金额/对方/日期/事项）后生成凭证或回答提问；
    生成凭证时 reply 中必须提醒「合同/报价单不能作为税前扣除凭据，请取得对应发票」；
    摘要注明依据（如「依据XX合同付首款」）；合同约定分期付款时只按当前要入账的金额生成，其余说明后续再来。
11. 最终回答只输出一个 JSON 对象，格式：
